...
else
{
	...
	// prepare list filter
	// если тут в PROPERTY_STATE  прописать сразу число соотв уволенени, переводу и тд -то отображает именно как надо.
	 $arFilter = array('ACTIVE' => 'Y', 'ACTIVE_DATE' => 'Y', 'IBLOCK_ID' => $arParams['IBLOCK_ID'], '!PROPERTY_USER' => false, 'PROPERTY_STATE'=>'');

	if ($DEPARTMENT > 0)
		$arFilter['PROPERTY_DEPARTMENT'] = CIntranetUtils::GetIBlockSectionChildren(intval($DEPARTMENT));
// add filter STATE- status -so we can filter data 	
	if ($STATE > 0)
		$arFilter['PROPERTY_STATE'] =$STATE;
